...
#ifdef track1_cxx
track1::track1(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
     tree=(TTree*)gDirectory->Get("track1");
   }
   Init(tree);
}
...
Int_t track1::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t track1::LoadTree(Long64_t entry)
{
...
void track1::Show(Long64_t entry)
{
...
Int_t track1::Cut(Long64_t entry)
{
