...
void Phi0VsPvb(char *filename="", int TrackClassCut=4)
{
	...
	sprintf(HRSCut,"abs((P0-Pvb)/P0)<0.05 && abs(Thetavb_tr)<0.051 && abs(Phivb_tr)<0.045");

	TCanvas *pCan=new TCanvas("pCan","Phi0 in G2P",800,600);
	gPad->SetRightMargin(0.05);
	
	track0->Draw("Phi0*180/3.14159:P0>>h2Phi0P0(175,0.,3.50,150,-60,90)",HRSCut,"prof");
	h2Phi0P0->SetMarkerStyle(20);
	h2Phi0P0->SetMarkerColor(1);
	h2Phi0P0->SetTitle(Form("#phi_{0} Vs P_{0} for HRS=%.2f^{o}, Y_{offset}=%.1f mm; P_{0} (GeV); #phi_{0} (deg)",LHRSAngle*180./3.14159,TargetYOffset));
	h2Phi0P0->GetYaxis()->SetRangeUser(-60,90);
	h2Phi0P0->Draw("prof");
	...
	pCan->cd();
	pCan->SaveAs(Form("PhiVsP_HRS%.2f.gif",LHRSAngle*180./3.14159));	
 }
 
 
void Phi0_trVsPvb(char *filename="", int TrackClassCut=4)
{
	gStyle->SetPadGridX(1);
	gStyle->SetPadGridY(1);
	gStyle->SetOptFit(1);
	...
	track0->Draw("Phi0_tr*180/3.14159:P0>>h2Phi0_trP0(175,0.,3.50,150,-60,90)",HRSCut,"prof");
	h2Phi0_trP0->SetMarkerStyle(20);
	h2Phi0_trP0->SetMarkerColor(1);
	h2Phi0_trP0->SetTitle(Form("#phi_{0} Vs P_{0} for HRS=%.2f^{o}, Y_{offset}=%.1f mm; P_{0} (GeV); #phi_{0} (deg)",LHRSAngle*180./3.14159,TargetYOffset));
	h2Phi0_trP0->GetYaxis()->SetRangeUser(-5,5);
	h2Phi0_trP0->Draw("prof");
	h2Phi0_trP0->Fit(f2,"R","", 0.3,3.5);
	track0->Draw("Phi0_tr*180/3.14159:P0",HRSCut,"same");
	...
	pCan->cd();
	pCan->SaveAs(Form("Phi0_trVsP_HRS%.2f.gif",LHRSAngle*180./3.14159));	
 }
 
void Theta0VsPvb(char *filename="", int TrackClassCut=4)
{
	gStyle->SetPadGridX(1);
	gStyle->SetPadGridY(1);
	gStyle->SetOptFit(1);
	...
	h2Theta0P0->SetTitle(Form("#theta_{0} Vs P_{0} for HRS=%.2f^{o}, Y_{offset}=%.1f mm; P_{0} (GeV); #theta_{0} (deg)",LHRSAngle*180./3.14159,TargetYOffset));
   ...
   leg->Draw();
	...
	pCan->cd();
	pCan->SaveAs(Form("ThetaVsP_HRS%.2f.gif",LHRSAngle*180./3.14159));	
 }
 
void Theta0VsPvb_newfcn(char *filename="", int TrackClassCut=4)
{
	gStyle->SetPadGridX(1);
	gStyle->SetPadGridY(1);
	gStyle->SetOptFit(1);
   ...
   leg->Draw();
	...
	pCan->cd();
	pCan->SaveAs(Form("ThetaVsP_HRS%.2f_newfcn.gif",LHRSAngle*180./3.14159));	
 }
 ////////////////////////////////////////////////////////////////
 
void Theta0_trVsPvb(char *filename="", int TrackClassCut=4)
{
	gStyle->SetPadGridX(1);
	gStyle->SetPadGridY(1);
	gStyle->SetOptFit(10);
	...
	h2Theta0_trP0->SetTitle(Form("#theta_{0}^{tr} Vs P_{0} for HRS=%.2f^{o}, Y_{offset}=%.1f mm; P_{0} (GeV); #theta_{0}^{tr} (deg)",LHRSAngle*180./3.14159,TargetYOffset));
	...
	pCan->cd();
	pCan->SaveAs(Form("Theta0_trVsP_HRS%.2f.gif",LHRSAngle*180./3.14159));	
 }
...
void PlotQ2VsP0(char *filename="", int TrackClassCut=4)
{
	...
	for(int i=0;i<4;i++)
	{
		sprintf(strQ2[i],"2.0*%.3f*P0*(1.0-cos(Theta0)):P0>>h2Q2P0_%d(175,0.,3.50,30,0,0.15)",
		BeamEnergy[i],i);
	}
	
	char HRSCut[255];
	sprintf(HRSCut,"abs((P0-Pvb)/P0)<0.05 && TrackClass>=%d",TrackClassCut);
	//SNAKE's preliminary cut at septum entrance plane
	sprintf(HRSCut,"abs((P0-Pvb)/P0)<0.05 && abs(Thetavb_tr)<0.051 && abs(Phivb_tr)<0.045");
	...
	pCan->cd();
	pCan->SaveAs(Form("Q2VsP0_HRS%.2f.gif",LHRSAngle*180./3.14159));		
 }
 
 
 void ThetaVsPvb_YOffset_Cut(char *filename="", int TrackClassCut=4, const char *key="")
 {
	 //plot them all
	 Phi0_trVsPvb(filename,TrackClassCut); return;
	 Phi0VsPvb(filename,TrackClassCut);
	 Theta0VsPvb(filename,TrackClassCut);
	 Theta0VsPvb_newfcn(filename,TrackClassCut);
	 Theta0_trVsPvb(filename,TrackClassCut);
	 PlotQ2VsP0(filename,TrackClassCut);
		 
	if(strlen(key)>1)
	{	 
		char strHRS[255];
		sprintf(strHRS,"HRS%.2f",LHRSAngle*180./3.14159);
		system(Form("copy /Y Theta0_trVsP_%s.gif Theta0_trVsP_%s_%s.gif",strHRS,strHRS,key));
		system(Form("copy /Y ThetaVsP_%s.gif ThetaVsP_%s_%s.gif",strHRS,strHRS,key));
		system(Form("copy /Y ThetaVsP_%s_newfcn.gif ThetaVsP_%s_newfcn_%s.gif",strHRS,strHRS,key));
		system(Form("copy /Y PhiVsP_%s.gif PhiVsP_%s_%s.gif",strHRS,strHRS,key));		
		system(Form("copy /Y Phi0_trVsP_%s.gif PhiVsP_%s_%s.gif",strHRS,strHRS,key));
		system(Form("copy /Y Q2VsP0_%s.gif Q2VsP0_%s_%s.gif",strHRS,strHRS,key));
		
		system(Form("del /F  Theta0_trVsP_%s.gif ",strHRS));
		system(Form("del /F  ThetaVsP_%s.gif ",strHRS));
		system(Form("del /F  ThetaVsP_%s_newfcn.gif ",strHRS));
		system(Form("del /F  PhiVsP_%s.gif ",strHRS));
		system(Form("del /F  Phi0_trVsP_%s.gif ",strHRS));
		system(Form("del /F  Q2VsP0_%s.gif ",strHRS));
	}
