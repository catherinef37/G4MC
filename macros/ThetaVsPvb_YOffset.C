...
void PlotQ2VsP0(char *filename="", int TrackClassCut=4)
{
	...
	for(int i=0;i<4;i++)
	{
		...
		h2->SetTitle(Form("Beam=%.3f GeV, HRS_Acc_Cut=%d; P_{0} (GeV); Q^{2} (GeV^{2})",
		BeamEnergy[i],TrackClassCut));	
		h2->Draw("contz");
	}
	pCan->cd();
 ...
 void ThetaVsPvb_YOffset(char *filename="", int TrackClassCut=4, const char *key="")
 {
	 //plot them all
