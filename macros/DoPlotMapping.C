...
void SetThisStyle()
{
	...
	gStyle->SetTitleX(gStyle->GetPadLeftMargin()+0.02);
	gStyle->SetTitleW(1.0-gStyle->GetTitleX()-0.02);
...
void DoPlotMapping(TCut theCut,int holeindex)
{
  SetThisStyle();
	...
	double yy=1.0-gPad->GetTopMargin()-0.01; 
	TPaveText *pt = new TPaveText(gPad->GetLeftMargin()+0.01,yy-0.8,xx,yy,"brNDC");
	pt->SetBorderSize(0);
	pt->SetFillColor(0);
	pt->SetTextColor(2);
	...
	{
	  //text=pt->AddText("NO Target Field");
		text=pt->AddText("Target Field =5.0 T");
	}
	//text=pt->AddText(Form("Beam = %.3f",Beam));
	text=pt->AddText(Form("Beam = %.3f",2.254));
	if(!bIsCombinedTree)
	{  
	  //text=pt->AddText(Form("Target_A = %.0f",TargetAtomicNumber));
		text=pt->AddText("3cm NH3+He4 Target");
		text=pt->AddText("2cm Raster");
	} 
	else
	{  
		text=pt->AddText("Multi-Targets");	
	...
	{  
		pMapCan->SaveAs(Form("graph/SieveHole%02d_E%.3f_Helm%.0fdeg_R%.1f_A%.0f_%s.png",holeindex,
			Beam,HelmRotAngle1*180/3.141593,HelmCurrentRatio,TargetAtomicNumber,gKey));	
		pMapCan->SaveAs(Form("graph/SieveHole%02d_E%.3f_Helm%.0fdeg_R%.1f_A%.0f_%s.C",holeindex,
			Beam,HelmRotAngle1*180/3.141593,HelmCurrentRatio,TargetAtomicNumber,gKey));	
	} 
	else
	{ 
		pMapCan->SaveAs(Form("graph/SieveHole%02d_E%.3f_Helm%.0fdeg_R%.1f_combined_%s.png",holeindex,
			Beam,HelmRotAngle1*180/3.141593,HelmCurrentRatio,gKey));
		pMapCan->SaveAs(Form("graph/SieveHole%02d_E%.3f_Helm%.0fdeg_R%.1f_combined_%s.C",holeindex,
			Beam,HelmRotAngle1*180/3.141593,HelmCurrentRatio,gKey));
	}
}
