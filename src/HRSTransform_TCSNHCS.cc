...
namespace Transform
{
	...
	//transorm from Hall coordinate to tranportation coordinate 
	//the the situation that the "origin point" is not (0,0,0)
	void X_HCS2TCS(double x, double y, double z, double EndPlaneX_hall, 
		double EndPlaneY_hall, double EndPlaneZ_hall, double EndPlaneTheta_hall,  
		double &x_tr, double &y_tr, double &z_tr)
	{
		//definition
		//x_tr = -y;
		//y_tr =  x * cosHRS - z * sinHRS;
		//z_tr =  x * sinHRS + z * cosHRS;  
		//Theta_tr=atan(dx/dz)=atan(-y/(x * sinHRS + z * cosHRS))
		//Phi_tr=atan(dy/dz)=atan((x * cosHRS - z * sinHRS)/(x * sinHRS + z * cosHRS))
		double cosHRS=cos(EndPlaneTheta_hall), sinHRS=sin(EndPlaneTheta_hall);

		//do the translation such that the pivot is at the intercept point of the central ray and the EndPlane 
		//y unchange, but x and z shifted
		x -= EndPlaneX_hall;
		y -= EndPlaneY_hall;
		z -= EndPlaneZ_hall;
