...
HRSMultScatt::HRSMultScatt() {
    InitInternal();

    return;
}

HRSMultScatt::HRSMultScatt( double p, int nmat, double t[], double A[], double Z[] ){
    /*
       p    - electron momentum
...
HRSMultScatt::HRSMultScatt( double p, double t, double A, double Z ){
    /*
       p    - electron momentum
       nmat - number of materials
       t    - Thickness
...
void HRSMultScatt::InitInternal(){
    fInit = false;
    fErf2sig = erf(2.0/sqrt(2.0));
    fNmat = 0;
}

void HRSMultScatt::Init( double p, int nmat, double t[], double A[], double Z[] ){
    /* 
       ...
       Z    - Atomic number
    */
    //G4cout << p << " " << nmat << G4endl;
    //for(int i = 0; i < nmat; i++){
  //G4cout << t[i] << " " << A[i] << " " << Z[i] << G4endl;
  // }
    ...
    for( i = 0; i < nmat; i++ ){
	X0  = (716.4/10./10.)*A[i]/(Z[i]*(Z[i]+1.0)*log(287.0/sqrt(Z[i])));
	//G4cout << "X0, t: " << X0 << " " << t[i] << G4endl;
	radsum += t[i]/X0;
    }
    ...
    for( i = 0; i < fNmat; i++ ){
	expb_num = (6680.0*10*10)*ft[i]*(fZ[i]+1.0)*pow(fZ[i],1.0/3.0);
	expb_den = fA[i]*(1.0+3.34*pow(fZ[i]/137.0,2.0));

	bsum += expb_num/expb_den;
    }
    //G4cout << "radsum, bsum: " << radsum << " " << bsum << G4endl;
	...
	*/
    ...
    //G4cout << "b, bsum: " << b << " " << bsum << G4endl;
    fB = solvelogeq(b);
    //G4cout << "solving log eq: " << fB << G4endl;
    /////////////////////////////////////////
...
void   HRSMultScatt::Init( double p, double t, double A, double Z ){
    /*
...
double HRSMultScatt::J0(double x) {
...
double HRSMultScatt::solvelogeq(double b){
...
double HRSMultScatt::fn_integrand( double u, double th, int n ){
...
double HRSMultScatt::intsimpson_fn( double th, int n ){
...
double HRSMultScatt::CalcMSDistPlane( double theta, double p, double t, double A, double Z ){
    Init( p, t, A, Z );
    //G4cout << "Is it a function in a function, 5arg?" << G4endl;
    return CalcMSDistPlane(theta);
}

double HRSMultScatt::CalcMSDistPlane( double theta, double p, int nmat, double t[], double A[], double Z[] ){
    Init( p, nmat, t, A, Z );
    
    //G4cout << "Is it a function in a function, 6arg?" << G4endl;
    return CalcMSDistPlane(theta);
}

double HRSMultScatt::CalcMSDistPlane( double theta){
  //G4cout << "Inside CalcMSDistPlane: theta = " << theta << G4endl;
    /*
       p    - electron momentum
       nmat - number of materials
       t    - Thickness
    ...
    //G4cout << "fchi2, fB: " << fchi2 << " " << fB << G4endl;
...
double HRSMultScatt::CalcMSDist( double theta, double p, double t, double A, double Z ){
    Init( p, t, A, Z );
    return CalcMSDist(theta);
}

double HRSMultScatt::CalcMSDist( double theta, double p, int nmat, double t[], double A[], double Z[] ){
    Init( p, nmat, t, A, Z );
    return CalcMSDist(theta);
}

double HRSMultScatt::CalcMSDist( double theta){
  //G4cout << "Is this causing trouble?" << G4endl;
    return CalcMSDistPlane(theta)*sin(fabs(theta));
}

double HRSMultScatt::GenerateMSPlane(){
    /*
	...
	do {
	    ...
	    //G4cout << fl << " " << fth << " " << fDt << G4endl;
	    //G4cout << "trialv " << trialv << G4endl;
	    //G4cout << "Or is this causing trouble?" << G4endl;
	} 
...
double HRSMultScatt::GenerateMSPlane( double p, int nmat, double t[], double A[], double Z[] ){
    /*
       p    - electron momentum, [GeV]
       nmat - number of materials
       t    - Thickness [g/cm2]
...
double HRSMultScatt::GenerateMSPlane( double p, double t, double A, double Z ){
    /*
       p    - electron momentum
       nmat - number of materials
       t    - Thickness
...
double HRSMultScatt::GenerateMS(){
...
double HRSMultScatt::GenerateMS( double p, int nmat, double t[], double A[], double Z[] ){
    /*
       p    - electron momentum
       nmat - number of materials
       t    - Thickness 
...
double HRSMultScatt::GenerateMS( double p, double t, double A, double Z ){
    /*
       p    - electron momentum
       nmat - number of materials
       t    - Thickness
